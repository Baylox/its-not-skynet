...
### Tooling (`scripts/`)
Pure-shell, deterministic, no network (`jq` optional): `scripts/new.sh <type> <handle> <name>` scaffolds a resource (skills get a trigger-oriented description + `references/`); `scripts/install.sh <type>/<handle>/<name> [project]` installs a resource into a target project (`.claude/skills/<name>/`, `.claude/agents/`, `.claude/hooks/` + `settings.json` snippet); `scripts/validate.sh` lints (skills also: frontmatter, `name`==folder, description, broken/orphan links, line budget); `scripts/build-index.sh` regenerates the catalog + stats (`--check` verifies sync); `scripts/find.sh` searches by keyword/type/status/contributor; `scripts/audit-hooks.sh` security-scans hooks (append `# audit:allow` to silence a false positive); `scripts/doctor.sh` runs the full pre-PR check (and `--new <type> <handle> <name>` scaffolds → edits → lints in one go); `scripts/test.sh` tests the tooling.
